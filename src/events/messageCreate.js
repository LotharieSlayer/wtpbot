...
async function loadMemes(msg){
    ...
    const discussion = await getSetupData(msg.guild.id, "discussion")
...
async function isURL(msg){
	...
	if(isMatch && !isRole){
		...
		if(cpt < 2)
			msg.channel.send(`Attention <@${msg.author.id}> tu n'a pas le droit de poster des liens sans être certifié !\nMerci de bien vouloir lire le règlement.`)
